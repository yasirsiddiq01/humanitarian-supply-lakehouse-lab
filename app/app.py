...
def main():
    ...
    st.markdown(
        ...
        The project focuses on data ingestion, cleaning, validation,
        reconciliation, quality checks, and analytics-ready reporting.
        """
    )
    ...
    if missing_files:
        st.error("Some required output files are missing.")
        st.write("Run these scripts first:")
        st.code(
            """
python scripts/generate_synthetic_data.py
python scripts/validate_bronze_data.py
python scripts/create_silver_layer.py
python scripts/create_gold_layer.py
            """
        )
    ...
    warehouse_inventory_summary = load_csv(
        GOLD_DIR / "warehouse_inventory_summary.csv"
    )
    distribution_point_summary = load_csv(
        GOLD_DIR / "distribution_point_summary.csv"
    )
    ...
    col7.metric(
        "Delayed by Date",
        int(kpi["delayed_by_date_shipments"]),
    )
    col8.metric(
        "Delay Rate",
        f"{float(kpi['delayed_by_date_rate_percent']):.2f}%",
    )
    ...
    status_chart_data = shipment_status_summary.set_index("shipment_status")[
        "shipment_count"
    ]
    ...
    supplier_chart_data = supplier_performance.set_index("supplier_name")[
        "delayed_rate_percent"
    ]

    st.bar_chart(supplier_chart_data)

    st.header("5. Warehouse Inventory Summary")

    st.dataframe(warehouse_inventory_summary, use_container_width=True)

    inventory_chart_data = warehouse_inventory_summary.set_index(
        "warehouse_name"
    )["total_quantity_available"]

    st.bar_chart(inventory_chart_data)
    ...
    dq_chart_data = data_quality_summary.groupby("metric_name")[
        "metric_value"
    ].sum()

    st.bar_chart(dq_chart_data)

    st.header("8. Download Outputs")

    st.markdown("Download dashboard-ready Gold outputs:")
    ...
    st.caption(
        "Portfolio note: this is a local Pandas-based lakehouse simulation. "
        "It demonstrates migration logic, quality checks, reconciliation, "
        "and reporting continuity without claiming enterprise-scale Databricks implementation."
    )
